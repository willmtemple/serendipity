...
const Detached = observer(
  React.forwardRef<SVGGElement, DetachedProps>((props, ref) => {
        ...
        case "statement":
          return (
            <SvgFlex direction="vertical" padding={-10}>
              {(props.global as EditorDetachedStatements).element.map((_: unknown, idx: number) => (
                <Statement
                  key={idx}
                  fixed={idx === 0}
                  bind={props.global}
...
Detached.displayName = "DetachedSyntax";

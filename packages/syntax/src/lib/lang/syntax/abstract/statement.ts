...
export function matchStatement<T>(p: StatementMatcher<T>): Function<Statement, T> {
    return (s: Statement): T => {
        switch (s.statementKind) {
            case "print": return p.Print ? p.Print(s as Print) : p.Default(s);
            case "let": return p.Let ? p.Let(s as Let) : p.Default(s);
            case "if": return p.If ? p.If(s as If) : p.If(s);
            case "forin": return p.ForIn ? p.ForIn(s as ForIn) : p.Default(s);
